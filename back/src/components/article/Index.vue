<template>
    <div>
        ...
        <div class="section article">
            <h1><i class="icon icon-books"></i>文章列表<span class="add-icon" @click="add"><i class="icon-plus"></i>新增文章</span></h1>
            <div class="listTop">
                <div class="category">
                    选择分类：
            ...
            <transition name="init">
                <v-list class="init" type="" :cateId="cateId" :action="initAction" @init="loaded=true" @resetAction="initAction=false" @modify="modify" @deleteList="deleteList" v-show="listFlag"></v-list>
            </transition>
            <transition name="result">
                <v-list class="result" type="search" :cateId="cateId" :title="title" :action="searchAction" @resetAction="searchAction=false" @modify="modify" @deleteList="deleteList" v-if="searchFlag"></v-list>
            </transition>
        </div>
        <transition name="slide"><router-view class="handle" :category="category" :info="info" @update="getList"></router-view></transition>
    </div>
</template>
...
export default {
    ...
    methods : {
        ...
        deleteList(id){
            let url = '/api/article/delete/' + id, str = '文章';
...
.article{
    .listTop{
        ...
        .category{
            float: left;
            select{
                font-size: 12px; width: 100px; height: 26px; margin: -2px 10px 0 0; border-color: #ccc; vertical-align: middle;
...
.editor{
    position: relative;
    .errors{position: absolute; right: 10px; bottom: 5px; color: red; font-size: 12px; line-height: 20px;}
}
</style>
