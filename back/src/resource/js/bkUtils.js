const bkUtils = {
  ...
  //格式化日期
  formatDate: function (timestamp, flag) {
  ...
  //图片转为base64
  toBase64: function (vue, img, field) {
  ...
  //表单验证是否经过修改
  isModified: function (vue, arr, field) {
    let i = 0;
  ...
  //添加
  addList: function (vue, url, params) {
    vue.$ajax({
      method: 'post',
      url: url,
      headers: {
  ...
  //修改
  modifyList: function (vue, url, params) {
    vue.$ajax({
      method: 'put',
      url: url,
      headers: {
  ...
  //删除
  deleteList: function (vue, url, str) {
